...
# checks DataFrame is result of successful left outer merge, returns clean DataFrame
# requires a '_merge' column in df generated by setting indicator=True
...
def check_left_merge(df, check_type, left_var, right_var, logger, guidance="",):
    ...
    if check_type == 'any':
        if sum(df['_merge'] == 'left_only') > 0:
            logger.error(("TABLE JOIN ERROR: Join of {} with {} table ".format(left_var, right_var) +
                        "produced a mismatch. Check the corresponding table columns. {}".format(guidance)))
            raise Exception(("TABLE JOIN ERROR: Join of {} with {} table ".format(left_var, right_var) +
                            "produced a mismatch. Check the corresponding table columns. {}".format(guidance)))
    elif check_type == 'all':
        if sum(df['_merge'] == 'left_only') == df.shape[0]:
            logger.error(("TABLE JOIN ERROR: Join of {} with {} table ".format(left_var, right_var) +
                        "failed to produce any matches. Check the corresponding table columns. {}".format(guidance)))
            raise Exception(("TABLE JOIN ERROR: Join of {} with {} table ".format(left_var, right_var) +
                            "failed to produce any matches. Check the corresponding table columns. {}".format(guidance)))
    else:
        raise Exception("INPUT ERROR: {} is not a valid check_type value in check_left_merge method".format(check_type))
...
def generate_reports(dirLocation, cfg, logger):
    ...
    for log_file in log_files:
        ...
        the_date = datetime.datetime.strptime(the_file_name[-23:-4],
                                              "%Y_%m_%d_%H-%M-%S")
