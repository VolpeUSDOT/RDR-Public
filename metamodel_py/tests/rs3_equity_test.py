# Functional test of Reference Scenario 3 - Equity metrics
...
#   python -m pytest metamodel_py/tests/rs3_equity_test.py -v
...
def test_rs3():
    ...
    sys.path.insert(0, './helper_tools/equity_and_benefits_analysis/')
    import equity_config_reader

    # Change working directory to RDR/helper_tools/equity_and_benefits_analysis
    os.chdir(os.path.join(os.getcwd(), 'helper_tools', 'equity_and_benefits_analysis'))

    path_to_config = os.path.join(file_dir_path, 'RS3_equity_metrics.config')
    equity_cfg = equity_config_reader.read_equity_config_file(path_to_config)

    print(equity_cfg)

    equity_analysis_dir = os.path.normpath(equity_cfg['equity_analysis_dir'])

    print("equity_analysis_dir exists? {}".format(os.path.exists(equity_analysis_dir)))

    print(os.listdir(equity_analysis_dir))
    print(os.getcwd())
    ...
    # # Tests:
    # # - nrow of TAZ by origin and destination; should be identical to input Equity_TAZ_Mapping.csv
    ...
    # #-23.15032486
    # #-31.8499763
    assert os.path.exists(os.path.join(input_dir, 'Equity_TAZ_Mapping.csv'))
    mapping_file = pd.read_csv(os.path.join(input_dir, 'Equity_TAZ_Mapping.csv'))

    # assert os.path.exists(os.path.join(equity_analysis_dir, 'MetricsByTAZ_RS3EquityAnalysis.html'))
    # tazorigin_file = pd.read_csv(os.path.join(equity_analysis_dir, 'MetricsByTAZ_summary_RS3EquityAnalysis_byTAZofOrigin.csv'))
    # assert tazorigin_file.shape[0] == mapping_file.shape[0]

    # assert os.path.exists(os.path.join(equity_analysis_dir, 'MetricsByTAZ_summary_RS3EquityAnalysis_byTAZofDestination.csv'))
    # tazdest_file = pd.read_csv(os.path.join(equity_analysis_dir, 'MetricsByTAZ_summary_RS3EquityAnalysis_byTAZofDestination.csv'))
    # assert tazdest_file.shape[0] == mapping_file.shape[0]

    # assert os.path.exists(os.path.join(equity_analysis_dir, 'MetricsByTAZ_summary_RS3EquityAnalysis_byEquityCategory.csv'))
    # trips_percent_change_noresil_vals = [-23.1074, -31.6498]
    # tazcat_file = pd.read_csv(os.path.join(equity_analysis_dir, 'MetricsByTAZ_summary_RS3EquityAnalysis_byEquityCategory.csv'))

    # assert list(round(tazcat_file.trips_percent_change_noresil, 4)) == trips_percent_change_noresil_vals

    # # p-value test
    # report_file_path = os.path.join(equity_analysis_dir, 'MetricsByTAZ_RS3EquityAnalysis.html')
