# Functional test of Quick Start 4
# Run RDR Quick Start 4, evaluate outputs are as expected
# Local test:
#   conda activate RDRenv
#   cd C:/GitHub/RDR
#   pytest
# or to run just this file
#   python -m pytest metamodel_py/tests/qs4_full_test.py -v
...
def test_qs4(add_sample = True):
    # Run QS4
    returncode = call_qs4_bat()
    assert returncode == 0
    ...
    path_to_config = os.path.join(file_dir_path, 'QS4.config')
    ...
    assert os.path.exists(os.path.join(output_folder, 'full_combos_QS4.csv'))
    assert os.path.exists(os.path.join(output_folder, 'aeq_runs/base/QS4/base01/nocar/matrices/sp_base01.omx'))
    assert os.path.exists(os.path.join(output_folder, 'AequilibraE_Runs_Compiled_QS4.xlsx'))

    compiled_runs = pd.read_excel(os.path.join(output_folder, 'AequilibraE_Runs_Compiled_QS4.xlsx'),
                                  engine="openpyxl")
    ...
    # Assess results
    assert os.path.exists(os.path.join(output_folder, 'tableau_input_file_QS4.xlsx'))

    tableau_file = pd.read_excel(os.path.join(output_folder, 'tableau_input_file_QS4.xlsx'),
                                 engine="openpyxl")
