...
def main():
    ...
    program_description = 'Resilience Disaster Recovery Toll and Travel Time Helper Tool: ' \
    ...
    # Set up logging
    logger = rdr_supporting.create_loggers(output_dir, 'calc_time_and_toll', cfg)
    ...
    df_node = pd.read_csv(node_csv, usecols=['node_id', 'node_type'], converters={'node_id': int, 'node_type': str})
    ...
    df_link = pd.read_csv(link_csv, converters={'link_id': int, 'from_node_id': int, 'to_node_id': int, 'directed': int,
    ...
    if cfg['include_transit']:
        df_link['boarding_ind'] = np.where((df_link['from_node_type'] == 'transit boarding') & (df_link['to_node_type'] == 'transit'),
                                           1, 0)
        boarding_mode = df_link.loc[df_link['boarding_ind'] == 1, ['from_node_id', 'facility_type']]
        boarding_mode.rename({'from_node_id': 'boarding_node_id', 'facility_type': 'boarding_type'}, axis='columns', inplace=True)
        # boarding_mode = boarding_mode.drop_duplicates(subset=['boarding_node_id'], ignore_index=True)
        if boarding_mode['boarding_node_id'].duplicated().any():
            logger.error("Check transit boarding links. Same transit boarding node links to multiple transit modes")
            raise Exception("LINK CSV FILE ERROR: transit boarding links mix transit modes")

        df_link = pd.merge(df_link, boarding_mode, how='left', left_on='to_node_id', right_on='boarding_node_id')
        df_link['transfer_bus_ind'] = np.where((df_link['facility_type'] == 25) & (df_link['boarding_type'] == 23),
                                               1, 0)
        df_link['transfer_rail_ind'] = np.where((df_link['facility_type'] == 25) & (df_link['boarding_type'].isin([21, 22])),
                                                1, 0)
        ...
        # Bus wait time
        df_link['travel_time'] = np.where((df_link['boarding_ind'] == 1) & (df_link['facility_type'] == 23),
                                          df_link['travel_time'] + cfg['bus_wait_time'], df_link['travel_time'])
        df_link['travel_time'] = np.where(df_link['transfer_bus_ind'] == 1, df_link['travel_time'] + cfg['bus_wait_time'],
                                          df_link['travel_time'])

        # Tram/subway/rail wait time
        df_link['travel_time'] = np.where((df_link['boarding_ind'] == 1) & (df_link['facility_type'].isin([21, 22])),
                                          df_link['travel_time'] + cfg['subway_wait_time'], df_link['travel_time'])
        df_link['travel_time'] = np.where(df_link['transfer_rail_ind'] == 1, df_link['travel_time'] + cfg['subway_wait_time'],
                                          df_link['travel_time'])

        df_link.drop(labels=['boarding_ind', 'boarding_node_id', 'boarding_type', 'transfer_bus_ind', 'transfer_rail_ind'],
                     axis=1, inplace=True)
    ...
    if cfg['include_nocar']:
        ...
        # TNC cost per mile
        df_link['toll_nocar'] = np.where((~df_link['facility_type'].isin(['12', '21', '22', '23', '25'])) & ((df_link['from_node_type'] == 'road intersection') | (df_link['to_node_type'] == 'road intersection')),
