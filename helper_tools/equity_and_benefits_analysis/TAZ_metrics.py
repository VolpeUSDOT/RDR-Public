...
def run_notebook(full_path_to_config_file, equity_cfg, logger):
    ...
    if not os.path.exists(category_filename):
        logger.error('ERROR: {} not found. Please run the equity_overlay first or directly provide your own equity data file and specify the filename for it as TAZ_mapping in the equity_metrics.config file.'.format(category_filename))
    ...
    if taz_equity[category_name].dtype == 'O':
        logger.error("ERROR: Some of the data in {} are not numeric. The equity data must be binary or ordinal for the current version of this tool.".format(category_filename))
        raise Exception("ERROR: Some of the data in {} are not numeric. The equity data must be binary or ordinal for the current version of this tool.".format(category_filename))
