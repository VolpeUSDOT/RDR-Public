...
def read_equity_config_file(cfg_file):
    ...
    if not (cfg_dict['equity_source'].strip(' ').lower() == 'cejst'):
        if not os.path.exists(cfg_dict['equity_source']):
            raise Exception("RDR CONFIG FILE ERROR: {} could not be found".format(cfg_dict['equity_source']))
    cfg_dict['equity_feature'] = read_config_file_helper(cfg, 'equity_overlay', 'equity_feature', 'REQUIRED')
    cfg_dict['equity_crs'] = read_config_file_helper(cfg, 'equity_overlay', 'equity_crs', 'REQUIRED')
